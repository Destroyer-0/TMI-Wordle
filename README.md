# 夜雀食堂今天的饭

一款以料理属性为线索的中文每日猜谜游戏。玩家需要在八次机会内，根据厨具、价格、等级、食材、制作时间、标签、获得方式和 DLC 信息猜出目标料理。

项目使用原生 HTML、CSS 和 JavaScript 编写，无框架、数据库或后端服务，可直接部署到 GitHub Pages。

项目仓库：[Destroyer-0/TMI-Wordle](https://github.com/Destroyer-0/TMI-Wordle)

## 游戏特色

- 每日挑战：以 2026 年 1 月 1 日为第 1 期，按北京时间每天 00:00 更新。
- 无限模式：随时随机开始新一局。
- 每局最多猜测 8 次。
- 输入料理名称时显示匹配候选，禁止提交题库外料理或重复猜测。
- 比较厨具、价格、等级、食材、制作时间、正反 Tag、获得方式与 DLC。
- 绿色表示完全相同，黄色表示多值属性部分重合，灰色表示不同。
- 价格、等级和制作时间通过 `↑`、`↓` 提示答案更高或更低。
- 自动保存每日进度、无限模式进度和游戏统计。
- 提供玩法说明、统计与不泄露答案的彩色方块分享结果。
- 支持键盘操作、手机布局和基础无障碍功能。

## 题库

题库位于 [`data.js`](./data.js)，目前包含本体及 DLC1–DLC5 共 163 道料理。

每条料理记录使用以下结构：

```js
{
  id: "recipe-001",
  name: "料理名称",
  cookware: "煮锅",
  price: 8,
  level: 1,
  ingredients: ["海苔"],
  time: 6,
  time60: 3.6,
  positiveTags: ["素", "家常"],
  negativeTags: ["重油"],
  acquisition: "初始拥有",
  dlc: "本体"
}
```

`id` 和料理名称应保持唯一。食材与 Tag 使用数组；没有反向 Tag 时使用空数组。制作时间会显示为 `time（time60）`，例如 `3（1.8）`。

## 本地运行

在项目目录启动一个静态文件服务器：

```powershell
python -m http.server 8000
```

然后访问 `http://localhost:8000/`。

直接打开 `index.html` 也可以游玩，但部分浏览器可能会限制剪贴板分享功能。

## 项目结构

```text
.
├── index.html   页面结构
├── style.css    视觉样式与响应式布局
├── game.js      游戏逻辑与本地存档
├── data.js      料理题库
├── BG.png       页面背景
├── logo.png     网站图标
├── github-mark.svg  GitHub 仓库按钮图标
└── .nojekyll    GitHub Pages 静态发布标记
```

## 数据与版权说明

本项目为非官方粉丝制作项目，仅用于交流与娱乐，不隶属于或代表原作及其权利方。游戏名称、料理资料及相关内容的权利归各自权利方所有。
